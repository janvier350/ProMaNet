...
<html>
  ...
  <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.4.1/js/bootstrap.min.js"></script>
    ...
    <table class="table table-striped ">
        <thead >
            <tr>
                <th class="text-center" style="font-size: 16px">Opciones</th>
            
           <h3 class="text-center">Listado de Tareas a Realizar de cada Usuario Asignado </h3> 
           </tr>
        </thead>
        <tbody>
            <tr>
...
<!--MODAL PARA INSERTAR DETALLE DE TRABAJO Y ASIGNAR AL ASISTENTE  (SOLO JEFES)-->
   <div class="modal fade" id="myModal" role="dialog">
      ...
      <form  action="TODO_InsertTrabajo.jsp?idCab=<%=cabTrab%>&idTrabajo=<%=idTrabajo%>"  method="POST" >
                     ...
                     <div class="col-lg-12 form-group ">
                        ...
                        <select class="form-control" name="usuario">
                           <%try{
                            DriverManager.registerDriver(new oracle.jdbc.driver.OracleDriver());
                            Connection   cn = DriverManager.getConnection(url, user, pass);
                            String sql3 = "select * from usuario where estado = 'a' order by 3";
...
<!--                        <label for="Trabajo" class="form-control-label">Descripcion de la Tarea:</label>
                        ...
                        <select class="form-control" id="Trabajo" name="Trabajo" >
                           <%try{
                            DriverManager.registerDriver(new oracle.jdbc.driver.OracleDriver());
                            Connection   cn = DriverManager.getConnection(url, user, pass);
                            String sql3 = "SELECT * FROM TODO_DET_TAREAS WHERE ID_TODO_CAB_TAREAS = "+idTrabajo+" AND estado = 'A' order by 1";
                            PreparedStatement st = cn.prepareStatement(sql3);
                            ResultSet rs = st.executeQuery();       
                            while (rs.next()) {%>                                                                    

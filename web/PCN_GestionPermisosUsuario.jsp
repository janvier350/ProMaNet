<%--
    Document   : Gestion de permisos individuales por usuario (APP_USUARIO_PERMISO)
--%>
...
<head>
    ...
    <title>ProMaNet - Permisos por Usuario</title>
    ...
    <style>
        ...
        .badge-por-cargo-si{background:#e5f8ee;color:#1a7a4c;}
        .badge-por-cargo-no{background:#f4f5f7;color:#8898aa;}
    </style>
</head>
...
<aside class="sidenav bg-white navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-3 fixed-start ms-4" id="sidenav-main">
    ...
    <div class="collapse navbar-collapse w-auto" id="sidenav-collapse-main">
        <ul class="navbar-nav">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="PCN_GestionPermisosUsuario.jsp?idUser=<%=idUser%>">
                    <div class="icon icon-shape icon-sm border-radius-md text-center me-2 d-flex align-items-center justify-content-center">
            ...
            <li class="nav-item">
                <a class="nav-link" href="PCN_MatrizAccesos.jsp">
                    <div class="icon icon-shape icon-sm border-radius-md text-center me-2 d-flex align-items-center justify-content-center">
    ...
    <div class="sidenav-footer mx-3">
        <div class="card card-plain shadow-none" id="sidenavCard">
            ...
            <div class="card-body text-center p-3 w-100 pt-0">
                <div class="docs-info">
                    <h6 class="mb-0">Excepcion individual</h6>
                    <p class="text-xs font-weight-bold mb-0">Siempre gana al final</p>
                </div>
            </div>
        </div>
...
<main class="main-content position-relative border-radius-lg">
        ...
        <div class="container-fluid py-1 px-3">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb bg-transparent mb-0 pb-0 pt-1 px-0 me-sm-6 me-5">
                    ...
                    <li class="breadcrumb-item text-sm text-white active" aria-current="page">Permisos por Usuario</li>
                </ol>
                <h6 class="font-weight-bolder text-white mb-0">Permisos por Usuario</h6>
            </nav>
    ...
    <div class="container-fluid py-4">
        ...
        <a href="PCN_ListadoUsuario.jsp" class="btn btn-sm btn-outline-secondary mb-3"><i class="fa fa-arrow-left me-1"></i>Volver al listado</a>
        ...
        <%
            Connection cnHead = null;
            try {
                DriverManager.registerDriver(new oracle.jdbc.driver.OracleDriver());
                cnHead = DriverManager.getConnection(url, user, pass);

                PreparedStatement stUser = cnHead.prepareStatement(
                ...
                PreparedStatement stTot = cnHead.prepareStatement("SELECT COUNT(*) FROM APP_PERMISO WHERE ESTADO = 'A'");
                ResultSet rsTot = stTot.executeQuery();
                if (rsTot.next()) totalPermisos = rsTot.getInt(1);
                rsTot.close(); stTot.close();

                PreparedStatement stExc = cnHead.prepareStatement("SELECT COUNT(*) FROM APP_USUARIO_PERMISO WHERE IDUSUARIO = ?");
                stExc.setInt(1, idUser);
                        ...
                        <div class="row">
                            <div class="col-8">
                                <div class="numbers">
                                    ...
                                    <h5 class="font-weight-bolder mb-0"><%=nombreUsuario%> <%=apellidosUsuario%></h5>
                                    <span class="text-xs text-secondary">Cargo: <%=cargoUsuario%></span>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-8">
                                <div class="numbers">
                                    ...
                                    <h5 class="font-weight-bolder mb-0"><%=totalExcepciones%></h5>
                                </div>
                            </div>
                            <div class="col-4 text-end">
                        ...
                        <div class="row">
                            <div class="col-8">
                                <div class="numbers">
                                    ...
                                    <h5 class="font-weight-bolder mb-0"><%=totalPermisos%></h5>
                                </div>
                            </div>
                            <div class="col-4 text-end">
        ...
        <div class="card mt-4">
            <div class="card-body p-3">
                ...
                <div class="alert alert-info mt-3 mb-0 py-2 px-3" role="alert">
                    ...
                    <small>Estos permisos son excepciones puntuales para <b><%=nombreUsuario%> <%=apellidosUsuario%></b>: se suman o se restan de lo que su cargo (y su departamento) ya permiten, sin tener que cambiarle el cargo. Una excepcion aqui siempre gana al final. El cambio se aplica la proxima vez que esta persona inicie sesion.</small>
                </div>
            </div>
        </div>

        <form method="post" action="PCN_GuardarPermisosUsuario" id="formPermisos">
            <input type="hidden" name="idUser" value="<%=idUser%>">
            <%
                try {
                    Connection cn = cnHead;
                        ...
                        <table class="table align-items-center mb-0">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-uppercase text-xs font-weight-bolder opacity-7 text-center">Por su cargo</th>
                                    <th class="text-uppercase text-xs font-weight-bolder opacity-7 text-end pe-4">Asignacion para esta persona</th>
                                </tr>
                            </thead>
                            <tbody>
                            <%
                            ...
                            %>
                                <tr class="permiso-row" data-search="<%=codigo.toLowerCase()%> <%=descripcion.toLowerCase()%>">
                                    ...
                                    <td class="text-center">
                                        <span class="badge <%=porRol ? "badge-por-cargo-si" : "badge-por-cargo-no"%>"><%=porRol ? "Si" : "No"%></span>
                                    </td>
                                    <td class="text-end pe-4">
                                        <div class="btn-group btn-group-sm" role="group">
                                            <input type="radio" class="btn-check" name="permiso_<%=idPermiso%>" id="p<%=idPermiso%>_r" value="ROL" autocomplete="off" <%=("ROL".equals(tipoActual))?"checked":""%>>
                                            <label class="btn btn-outline-secondary" for="p<%=idPermiso%>_r">Segun su cargo</label>
                                            ...
                                            <label class="btn btn-outline-success" for="p<%=idPermiso%>_g">Conceder siempre</label>
                                            ...
                                            <label class="btn btn-outline-danger" for="p<%=idPermiso%>_d">Denegar siempre</label>
                                        </div>
                                    </td>
                                </tr>
                            <%
            ...
            <div class="card mt-4" id="barraGuardar">
                <div class="card-body p-3 d-flex justify-content-between align-items-center flex-wrap" style="gap:10px;">
                    <span class="text-sm text-secondary"><i class="fa fa-info-circle me-1"></i>Los cambios se aplican al guardar.</span>

...
<%
    ...
    boolean puedeImprimirComprobante = COMUN.PermisoHelper.tiene(session, "INVENTARIO_IMPRIMIR_COMPROBANTE");
    ...
    if (!"PENDIENTE".equals(filtroEstado) && !"ENTREGADO".equals(filtroEstado)) filtroEstado = "TODOS";
    String whereEstado = "TODOS".equals(filtroEstado) ? "" : " AND e.ESTADO_SOLICITUD = '" + filtroEstado + "'";

    boolean tieneDepto = idDepartamento != null && !idDepartamento.trim().isEmpty();

    String sqlLista =
        "SELECT e.ID_SUMINISTRO_EGRESO_CAB, e.FECHA_SOLICITUD, e.ESTADO_SOLICITUD, " +
        "       u.NOMBRE || ' ' || u.APELLIDOS AS SOLICITANTE, " +
        ...
        "WHERE e.ID_DEPARTAMENTO = ?" + whereEstado +
...
<head>
    ...
    <title>ProMaNet - Historial de mi Departamento</title>
...
<div class="sidebar">
    <nav class="nav flex-column pt-3">
        ...
        <% } %>
        <a class="nav-link active" href="INV_Historial_Solicitudes_Departamento.jsp"><i class="fa fa-building mr-2"></i>Historial por Departamento</a>
...
<div class="main-content">
    <div class="page-header">
        <ol class="breadcrumb"><li class="breadcrumb-item"><a href="#">Menu</a></li><li class="breadcrumb-item"><a href="#">Inventario</a></li><li class="breadcrumb-item active">Historial por Departamento</li></ol>
        <h4><i class="fa fa-building mr-2"></i>Historial de Suministros &mdash; <%=departamento != null ? departamento : "Mi Departamento"%></h4>
    </div>
    <div class="content-area">
        ...
        <div class="alert alert-warning">
            ...
            No se pudo determinar su departamento asignado. Contacte al administrador del sistema.
        </div>
        <% } else { %>
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <span class="font-weight-bold" style="font-size:.95rem;"><i class="fa fa-clipboard mr-1"></i>Solicitudes de <%=departamento != null ? departamento : "mi departamento"%> &mdash;
            ...
            <div class="card-body p-0">
                <div class="table-responsive">
                    <table class="table table-hover mb-0">
                        <thead><tr><th class="pl-3">#</th><th>Fecha</th><th>Solicitante</th><th>Autorizado por</th><th class="text-center">&Iacute;tems</th><th class="text-center">Estado</th><th class="text-center">Acciones</th></tr></thead>
                        <tbody>
                        <%
                            try {
                                DriverManager.registerDriver(new oracle.jdbc.driver.OracleDriver());
                                ...
                                st.setInt(1, Integer.parseInt(idDepartamento.trim()));
                                ...
                                while (rs.next()) {
                                    ...
                                    String solic = rs.getString(4) != null ? rs.getString(4) : "—";
                                    String autor = rs.getString(5) != null ? rs.getString(5) : "—";
                                    int    items = rs.getInt(6);
                        %>
                            <tr>
                                <td class="pl-3 font-weight-bold text-secondary"><%=idCab%></td>
                                <td><%=fecha%></td><td><%=solic%></td><td><%=autor%></td>
                        ...
                        <%
                                ...
                                if (!hayRegistros) {
                        %>
                            <tr><td colspan="7" class="text-center text-muted py-4"><i class="fa fa-inbox fa-2x mb-2 d-block"></i>No hay solicitudes registradas para su departamento.</td></tr>
                        <%  }
                                rs.close(); st.close(); cn.close();
                            } catch (Exception ex) { ex.printStackTrace();
                        %>
                            <tr><td colspan="7" class="text-center text-danger py-3"><i class="fa fa-exclamation-triangle mr-1"></i>Error: <%=ex.getMessage()%></td></tr>
                        <% } %>
                        </tbody>
                    </table>
                </div>
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row mb-3">
                    <div class="col-md-6"><small class="text-muted">Fecha</small><div id="modalFecha" class="font-weight-bold"></div></div>
                    <div class="col-md-6"><small class="text-muted">Solicitante</small><div id="modalSolic" class="font-weight-bold"></div></div>
                </div>
                <hr class="my-2">

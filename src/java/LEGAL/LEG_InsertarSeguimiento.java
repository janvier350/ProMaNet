...
@WebServlet(name = "LEG_InsertarSeguimiento", urlPatterns = {"/LEG_InsertarSeguimiento"})
public class LEG_InsertarSeguimiento extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (idLegalIp == null || descripcion == null || descripcion.trim().isEmpty()) {
        ...
        try {
            ...
            try (PreparedStatement stSec = cn.prepareStatement(
                    "SELECT NVL(MAX(ID_SEGUIMIENTO),0)+1 FROM LEGAL_IP_SEGUIMIENTO");
                 ResultSet rs = stSec.executeQuery()) {
                if (rs.next()) idNuevo = rs.getInt(1);
            }

            try (PreparedStatement st = cn.prepareStatement(
                    "INSERT INTO LEGAL_IP_SEGUIMIENTO (ID_SEGUIMIENTO, ID_LEGAL_IP, DESCRIPCION, ID_USUARIO_CREA) " +
                    "VALUES (?, ?, ?, ?)")) {
                st.setInt(1, idNuevo);

...
// Quick-add de materias desde el modal de EXPEL (AJAX), para no perder
// lo que el usuario ya escribio en el formulario principal.
@WebServlet(name = "LEG_InsertarMateria", urlPatterns = {"/LEG_InsertarMateria"})
public class LEG_InsertarMateria extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (descripcion == null || descripcion.trim().isEmpty()) {
            out.put("ok", false).put("mensaje", "Debe indicar el nombre de la materia.");
            response.getWriter().write(out.toString());
            return;
        }
        ...
        try {
            ...
            try (PreparedStatement stDup = cn.prepareStatement(
                    "SELECT ID_MATERIA FROM LEGAL_MATERIA WHERE UPPER(DESCRIPCION) = ? AND ESTADO = 'A'")) {
            ...
            try (PreparedStatement stSec = cn.prepareStatement(
                    "SELECT NVL(MAX(ID_MATERIA),0)+1 FROM LEGAL_MATERIA");
                 ResultSet rs = stSec.executeQuery()) {
                if (rs.next()) idNuevo = rs.getInt(1);
            }

            try (PreparedStatement st = cn.prepareStatement(
                    "INSERT INTO LEGAL_MATERIA (ID_MATERIA, DESCRIPCION) VALUES (?, ?)")) {
                st.setInt(1, idNuevo);
